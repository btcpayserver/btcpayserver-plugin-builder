...
public class NostrService(IMemoryCache cache, AdminSettingsCache adminSettingsCache)
{
    ...
    private static bool VerifyBip340(string pubkeyHex, string sigHex, byte[] msg32)
    {
        try
        {
            ...
            if (!ECXOnlyPubKey.TryCreate(pubKey, Context.Instance, out var xOnly)) return false;
    ...
    private async Task<JObject?> FetchFirstFromRelaysAsync(IEnumerable<NostrFilter> filters, TimeSpan timeout, Func<JObject, bool>? validate = null)
    {
        ...
        var tasks = adminSettingsCache.NostrRelays.Select(async relay =>
        {
            try
            {
                var list = await FetchEventsFromRelayAsync(relay, filterList, token, expected: 1).ConfigureAwait(false);
                var ev = list.Count > 0 ? list[0] : null;
                if (ev is null) return null;
                if (validate is not null && !validate(ev)) return null;
                return ev;
            }
            catch { return null; }
        }).ToList();
    ...
    private static async Task<IReadOnlyList<JObject>> FetchEventsFromRelayAsync(string relayUrl, IList<NostrFilter> filters, CancellationToken token, int expected = 1)
    {
        ...
        while (!token.IsCancellationRequested && ws.State == WebSocketState.Open)
        {
            ...
            if (JArray.Parse(msg) is not { Count: >= 2 } arr) continue;
            ...
            {
                results.Add(ev);
                if (results.Count < expected) continue;
    ...
    private Task<JObject?> FetchKind0FromRelaysAsync(string authorPubkeyHex, int timeoutMs = 6000)
    {
        ...
        return FetchFirstFromRelaysAsync(filters, TimeSpan.FromMilliseconds(timeoutMs),
            validate: ev => ev["kind"]?.Value<int>() == 0 && string.Equals((string?)ev["pubkey"], authorPubkeyHex, StringComparison.OrdinalIgnoreCase));
    }
    ...
    private static byte[] ConvertBits(byte[] data, int fromBits, int toBits, bool pad)
    {
        ...
        {
            if (v >> fromBits > 0) throw new ArgumentOutOfRangeException(nameof(data));
            acc = (acc << fromBits) | v; bits += fromBits;
            while (bits >= toBits) { bits -= toBits; ret.Add((byte)((acc >> bits) & maxV)); }
        }
...
{
    [JsonProperty("ids", NullValueHandling = NullValueHandling.Ignore)] public string[]? Ids { get; init; }
    [JsonProperty("kinds", NullValueHandling = NullValueHandling.Ignore)] public int[]? Kinds { get; init; }
    [JsonProperty("authors", NullValueHandling = NullValueHandling.Ignore)] public string[]? Authors { get; init; }
    [JsonProperty("limit", NullValueHandling = NullValueHandling.Ignore)] public int? Limit { get; init; }
}
