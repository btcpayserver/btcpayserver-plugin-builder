...
public class FeatureFlagTests(ITestOutputHelper logs) : UnitTestBase(logs)
{
    ...
    public async Task NewBuildsDisabled_StopsAlreadyQueuedBuildBeforeSandboxPreparation()
    {
        ...
        try
        {
            ...
            // Already accepted builds continue; this sandbox deliberately fails before upload.
            foreach (var blockerTask in blockerTasks)
            {
                var error = await Assert.ThrowsAsync<BuildServiceException>(
                    () => blockerTask.WaitAsync(TimeSpan.FromSeconds(10)));
                Assert.Equal("Plugin build failed.", error.Message);
            }
    ...
    [Fact]
    public async Task NewBuildsDisabled_DuringBrokerPreparation_AllowsAcceptedBuildToFinish()
    {
        ...
        try
        {
            ...
            Assert.Equal(BuildStates.Uploaded.ToEventName(), build.state);
            Assert.Null(build.error);
            Assert.NotNull(build.url);
            Assert.True(broker.Sandbox.Prepared[buildId].Started.Task.IsCompleted);
            ...
            Assert.Contains(BuildStates.Uploaded.ToEventName(), states);
            Assert.DoesNotContain(BuildStates.Failed.ToEventName(), states);
