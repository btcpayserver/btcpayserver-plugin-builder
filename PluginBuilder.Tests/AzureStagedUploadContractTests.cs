...
public class AzureStagedUploadContractTests
{
    ...
    {
        private readonly ConcurrentDictionary<string, byte[]> _storedArtifacts = new();
        ...
        public static async Task<BlobServer> Start(bool delayResponse = false, bool rejectRequest = false,
            Func<BlobRequest, CancellationToken, Task>? beforeResponse = null)
        {
            ...
            app.MapPut("/{**blob}", async (HttpContext context) =>
            {
                ...
                if (beforeResponse is not null)
                    await beforeResponse(request, context.RequestAborted);
                if (delayResponse)
                {
                    await Task.Delay(Timeout.InfiniteTimeSpan, context.RequestAborted);
                    return;
                }
                if (rejectRequest || request.IfNoneMatch == "*" &&
                    !server._storedArtifacts.TryAdd(request.Path, request.Body))
                {
                ...
                server._storedArtifacts[request.Path] = request.Body;
