    ...
    BuildExecutorOptions options, ILogger<BrokerCoordinator> logger) : BackgroundService
{
    ...
    public BrokerBuildStatus ReadStatus(string id, int cursor)
    {
        ...
        lock (lease.Gate)
        {
            ...
            return new(state, next, chunk.ToArray(), state == "succeeded" ? lease.Result : null,
    ...
    public void Start(string id)
    {
        ...
        lock (lease.Gate)
        {
            if (_stopping || !executor.Snapshot.IsReady || lease.Token.IsCancellationRequested || lease.State != "prepared")
                throw new BrokerRequestException(409, "The build lease is not ready to start.");
            lease.State = "running";
            lease.StartRequested.SetResult();
        }
    }
    ...
    private async Task RunAsync(Lease lease, FullBuildId id, BuildInfo info)
    {
        try
        {
            ...
            lock (lease.Gate) lease.State = "prepared";
            await lease.StartRequested.Task.WaitAsync(lease.Token);
            lease.Token.ThrowIfCancellationRequested();
            ...
            lease.Artifact = new FileStream(file.FullName, FileMode.Open, FileAccess.Read, FileShare.Read,
        ...
        catch (Exception error)
        {
            ...
            // returns no prepared handle: never turn that into a successful DELETE.
    ...
    private async Task CleanupAsync(Lease lease)
    {
        ...
        try
        {
            if (lease.CleanupFailed) throw new InvalidOperationException("Previous cleanup failed.");
    ...
    public async Task ReapExpiredAsync()
    {
        ...
        {
            try { await DeleteAsync(lease.Id); }
    ...
    private sealed class Lease : IOutputCapture
    {
        ...
        // Requests may retain a lease while another DELETE disposes its source.
        public readonly CancellationToken Token;
        public readonly SemaphoreSlim ArtifactGate = new(1, 1);
        public readonly TaskCompletionSource StartRequested = new(TaskCreationOptions.RunContinuationsAsynchronously);
